...
def test_from_registry_skips_contractless_tools() -> None:
    ...
    # action. As of F1.3, custom_cli, nmap, and nuclei are migrated, so they
    # are the only entries in the catalog; every other (contractless) wrapper
    # is absent.
    catalog = ToolCatalog.from_registry(build_default_registry())

    names = [tool.name for tool in catalog.tools]
    assert names == ["custom_cli", "nmap", "nuclei"]
